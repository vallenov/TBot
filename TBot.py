...
def tbot():
    ...
    while True:
        ...
        try:
            ...
            logger.exception(f'Infinity polling exception: {rt}\n{traceback.format_exc()}')
        except urllib3.exceptions.ReadTimeoutError as rte:
            logger.exception(f'Infinity polling exception: {rte}\n{traceback.format_exc()}')
        except socket.timeout as st:
            logger.exception(f'Infinity polling exception: {st}\n{traceback.format_exc()}')
        except Exception as ex:
            logger.exception(f'Infinity polling exception: {ex}\n{traceback.format_exc()}')
        finally:
            logger.info('Restart TBot')
            continue
